...
static int sockfd = -1, sockVolt = -1;

void __createFifoAmp() { 

	if (sockfd == -1) {
		sockfd = open("/home/max/tmp/reads.txt", O_WRONLY);
...
void __createFileVolt() { 

	if (sockVolt == -1) {
		sockVolt = open("/home/max/tmp/reads_volt.txt", O_WRONLY);

		printf("Writing V data\n");
	}

}

void __writeCurrent(const char *data) {

	__createFifoAmp();

	std::string msg = std::string(data) + std::string("\n");

	write(sockfd, msg.c_str(), msg.length());
}

void __writeVolt(const char *data) {

	__createFileVolt();

	std::string msg = std::string(data) + std::string("\n");

	write(sockVolt, msg.c_str(), msg.length());
}
...
void DMM::readVC820Continuous( ReadEvent *re )
{
  ...
  if (in[12] & 0x08) {
  	__writeCurrent(val.data());
  }  else if (in[12] & 0x04) {
	__writeVolt(val.data());
  }
